.. canopee-docs documentation master file, created by
   sphinx-quickstart on Tue Oct 11 11:55:17 2016.
   You can adapt this file completely to your liking, but it should at least
   contain the root `toctree` directive.

Documentation CANOPEE
=====================

Ce coin de Web regroupe toutes les documentations qui peuvent être utiles dans les missions quotidiennes d'un technicien d'études bâtiment.

.. note::
    La documentation est aussi disponible sous forme de document :download:`PDF <./docs/canopee-docs.pdf>` 
..    :download:`ici <./fichiers/gabarit_acad2010_teb+imprimante_pdf.zip>`



.. table::
  :class: main-content-presentation


  +--------------------------------------------------------------+--------------------------------------------------+
  |                                                              |                                                  |
  |  :ref:`Initiation Sketchup et AutoCAD <index_init_su+acad>`  |  :ref:`Photoshop et autres <index_ptshp>`        |
  |                                                              |                                                  |
  |  Les premiers pas ... et plus encore ...                     |  Quelques conseils pour enrichir vos dessins     |
  |                                                              |                                                  |
  +--------------------------------------------------------------+--------------------------------------------------+
  |                                                              |                                                  |
  |  :ref:`Sketchup <index_su>`                                  |  :ref:`FTP//WEB//Mail <index_ftpwebmail>`        |
  |                                                              |                                                  |
  |  Esquisses 3D en 3 clics                                     |  Le Web à usage professionnel                    |
  |                                                              |                                                  |
  +--------------------------------------------------------------+--------------------------------------------------+
  |                                                              |                                                  |
  |   :ref:`AutoCAD <index_acad>`                                |  :ref:`Bureautique <index_ftpwebmail>`           |
  |                                                              |                                                  |
  |   Aborder le maître en toute sérénité ...                    |  Écrits professionnels                           |
  |                                                              |                                                  |
  +--------------------------------------------------------------+--------------------------------------------------+



Vous y trouverez toutes sortes de documents, sous forme de :

*Guides pas à pas :*
  Réponse à une problématique concrète. Ces documents sont courts.

*Tutoriels :*
  Exemples complets, incluant des concepts de base et la pratique associée. Ces documents peuvent faire appel à certains guides, ils peuvent être assez longs.

*FAQ* &  *Glossaires*
  Recueil des questions les plus fréquemment posées.
  Liste des termes les plus fréquemment employés.


Sommaire complet :

.. toctree::
   :maxdepth: 2

   init_su+acad/index
   su/index
   acad/index
   psd/index
   ftpwebmail/index
   burotic/index
   

Pour toute erreur, commentaire positif, etc., écrivez moi :email:`Olivier TURLIER <o.turlier@gmail.com>` 

.. sur ``o point turlier at gmail point com``

Document en version |version| indice |release| du |today|

.. --------------------- pb with make latexpdf
.. fabriqué ici : 
.. .. googlemaps:: 6 Rue Jules Raimu, 13700 Marignane
.. , avec ça : 
..   .. youtube:: http://www.youtube.com/watch?v=Ql9sn3aLLlI
..  :width: 640
..  :height: 480
.. -----------------------------
